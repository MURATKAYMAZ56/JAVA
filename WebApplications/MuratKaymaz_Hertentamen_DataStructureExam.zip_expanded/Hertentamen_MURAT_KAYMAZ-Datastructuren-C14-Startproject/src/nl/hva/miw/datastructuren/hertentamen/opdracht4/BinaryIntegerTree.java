...
public class BinaryIntegerTree {
	...
	private int getLeafCount(TreeNode<Integer> node){ 
        if (node == null) 
            return 0; 
	...
	private int sumAllNodes(TreeNode<Integer> root)  {  
	    if (root == null)  
	        return 0;  
	    return (root.getValue() + sumAllNodes(root.getLeft()) + sumAllNodes(root.getRight()));  
	} 
}